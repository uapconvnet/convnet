...
namespace dnn
{
	...
	public:
		...
		void BackwardPropRef(const UInt batchSize)
		{
			...
			auto memDiffSrc = SharesInput ? dnnl::memory(bwdDesc->diff_src_desc(), Device.engine) : dnnl::memory(*InputLayerBwd->DiffDstMemDesc, Device.engine, InputLayerBwd->NeuronsD1.data());
