...
namespace dnn
{
	...
	public:
		...
		std::string GetDescription() const final override
		{
			...
			description.append(nwl + std::string(" Alpha:      ") + tab + FloatToString(Alpha));
			description.append(nwl + std::string(" Beta:       ") + tab + FloatToString(Beta));
				...
				if (plain)
				{
					...
					for_i(C, threads, [=](UInt c)
					{
						...
						for (auto n = 0ull; n < batchSize; n++)
						{
							...
							for (auto hw = start; hw < part; hw += VectorSize)
							{
								inputNeurons.load_a(&InputLayer->Neurons[hw]);
								inputNeurons -= Mean[c];
							...
							for (auto hw = part; hw < start + HW(); hw++)
							{
								diffSrcFloat = Func.df(((InputLayer->Neurons[hw] - Mean[c]) * weightedInvStdDev) + biases, Alpha, Beta) * layerD1[hw];
								KahanSum<Float>(diffSrcFloat * (InputLayer->Neurons[hw] - Mean[c]), diffGammaFloat, correction0Float);
						...
						if (InplaceBwd)
							for (auto n = 0ull; n < batchSize; n++)
							{
								...
								for (auto hw = start; hw < part; hw += VectorSize)
								{
									diffSrc = Func.dfVec((VecFloat().load_a(&InputLayer->Neurons[hw]) - Mean[c]) * weightedInvStdDev + biases, Alpha, Beta) * VecFloat().load_a(&layerD1[hw]);
					...
					for_i(PaddedC / VectorSize, threads, [=](UInt c)
					{
						...
						for (auto n = 0ull; n < batchSize; n++)
						{
							...
							for (auto h = 0ull; h < H; h++)
							{
								...
								for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
								{
									inputNeurons.load_a(&InputLayer->Neurons[w]);
									inputNeurons -= mean;
		...
		void BackwardPropRef(const UInt batchSize)
		{
			...
			if (InputLayerBwd->DstMemDesc->get_ndims() == 2)
			{
				...
				if (batchSize == 1)
				{
					if (InplaceBwd)
					{
						...
						{
							for (auto c = 0ull; c < PaddedC; c += VectorSize)
								(Func.dfVec(VecFloat().load_a(&InputNeurons[c]), Alpha, Beta) * VecFloat().load_a(&InputLayerBwd->NeuronsD1[c])).store_a(&InputLayerBwd->NeuronsD1[c]);
						}
						else
						{
							for (auto c = 0ull; c < C; c++)
								InputLayerBwd->NeuronsD1[c] = Func.df(InputNeurons[c], Alpha, Beta) * InputLayerBwd->NeuronsD1[c];
						}
					}
					else
					{
					...
					if (InplaceBwd)
					{
						if (!plain)
							for (auto c = 0ull; c < PaddedC; c += VectorSize)
							{
								...
								for (auto hw = offset; hw < offset + strideHW; hw += VectorSize)
									(Func.dfVec(VecFloat().load_a(&InputNeurons[hw]), Alpha, Beta) * VecFloat().load_a(&InputLayerBwd->NeuronsD1[hw])).store_a(&InputLayerBwd->NeuronsD1[hw]);
							}
						else
						{
							for (auto c = 0ull; c < C; c++)
							{
								const auto offset = c * HW();
								for (auto hw = offset; hw < offset + HW(); hw++)
									InputLayerBwd->NeuronsD1[hw] = Func.df(InputNeurons[hw], Alpha, Beta) * InputLayerBwd->NeuronsD1[hw];
							}
						}
					}
					else
						...
						if (!plain)
							...
							for_i(batchSize, threads, [=](UInt n)
							{
								for (auto c = 0ull; c < C; c++)
								{
									const auto offset = n * CDHW() + c * HW();
									for (auto hw = offset; hw < offset + HW(); hw++)
										InputLayerBwd->NeuronsD1[hw] *= Func.df(InputNeurons[hw], Alpha, Beta);
								}
							});
					}
					else
			...
			auto memDiffSrc = SharesInputInplace ? dnnl::memory(bwdDesc->diff_src_desc(), Device.engine) : dnnl::memory(*InputLayerBwd->DiffDstMemDesc, Device.engine, InputLayerBwd->NeuronsD1.data());
