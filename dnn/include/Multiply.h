				...
				if (EqualDimensions(Inputs))
				{
					...
					if (batchSize == 1)
					{
						if (!plain)
						{
							...
							{
								mul_add(VecFloat().load_a(&InputsFwd[1]->Neurons[cdhw]), VecFloat().load_a(&NeuronsD1[cdhw]), VecFloat().load_a(&InputsBwd[0]->NeuronsD1[cdhw])).store_a(&InputsBwd[0]->NeuronsD1[cdhw]);
								mul_add(VecFloat().load_a(&InputsFwd[0]->Neurons[cdhw]), VecFloat().load_a(&NeuronsD1[cdhw]), VecFloat().load_a(&InputsBwd[1]->NeuronsD1[cdhw])).store_a(&InputsBwd[1]->NeuronsD1[cdhw]);
							}
						}
						else
						{
							...
							for (auto cdhw = 0ull; cdhw < CDHW(); cdhw++)
							{
								InputsBwd[0]->NeuronsD1[cdhw] += NeuronsD1[cdhw] * InputsFwd[1]->Neurons[cdhw];
								InputsBwd[1]->NeuronsD1[cdhw] += NeuronsD1[cdhw] * InputsFwd[0]->Neurons[cdhw];
							}
						}
					}
					else
					{
						...
						if (!plain)
							for_i(batchSize, threads, [=](UInt n)
							{
								...
								for (auto c = 0ull; c < PaddedC; c += VectorSize)
								{
									...
									for (auto hw = start; hw < end; hw += VectorSize)
									{
										...
										mul_add(neuronsD1, VecFloat().load_a(&InputsFwd[second]->Neurons[hw]), VecFloat().load_a(&InputsBwd[first]->NeuronsD1[hw])).store_a(&InputsBwd[first]->NeuronsD1[hw]);
										mul_add(neuronsD1, VecFloat().load_a(&InputsFwd[first]->Neurons[hw]), VecFloat().load_a(&InputsBwd[second]->NeuronsD1[hw])).store_a(&InputsBwd[second]->NeuronsD1[hw]);
									}
								}
							});
						else
							for_i(batchSize, threads, [=](UInt n)
							{
								...
								for (auto cdhw = start; cdhw < end; cdhw++)
								{
									InputsBwd[first]->NeuronsD1[cdhw] += NeuronsD1[cdhw] * InputsFwd[second]->Neurons[cdhw];
									InputsBwd[second]->NeuronsD1[cdhw] += NeuronsD1[cdhw] * InputsFwd[first]->Neurons[cdhw];
								}
							});
#ifdef DNN_STOCHASTIC
					}
					...
					if (batchSize == 1)
					{
						if (!plain)
						{
							...
							for (auto c = 0ull; c < PaddedC; c += VectorSize)
							{
								...
								for (auto hw = 0ull; hw < strideHW; hw += VectorSize)
								{
									...
									mul_add(neuronsD1, VecFloat().load_a(&InputsFwd[second]->Neurons[c]), VecFloat().load_a(&InputsBwd[first]->NeuronsD1[hw + outputOffset])).store_a(&InputsBwd[first]->NeuronsD1[hw + outputOffset]);
									mul_add(neuronsD1, VecFloat().load_a(&InputsFwd[first]->Neurons[hw + outputOffset]), VecFloat().load_a(&InputsBwd[second]->NeuronsD1[c])).store_a(&InputsBwd[second]->NeuronsD1[c]);
								}
							}
						}
						else
						{
							for (auto c = 0ull; c < C; c++)
							{
								...
								for (auto hw = 0ull; hw < HW(); hw++)
								{
									InputsBwd[first]->NeuronsD1[hw + outputOffset] += NeuronsD1[hw + outputOffset] * InputsFwd[second]->Neurons[c];
									InputsBwd[second]->NeuronsD1[c] += NeuronsD1[hw + outputOffset] * InputsFwd[first]->Neurons[hw + outputOffset];
								}
							}
						}
					}
						...
						if (!plain)
							for_i(batchSize, threads, [=](UInt n)
							{
								...
								for (auto c = 0ull; c < PaddedC; c += VectorSize)
								{
									...
									{
										neuronsD1.load_a(&NeuronsD1[hw]);
										mul_add(neuronsD1, VecFloat().load_a(&InputsFwd[second]->Neurons[channelOffset]), VecFloat().load_a(&InputsBwd[first]->NeuronsD1[hw])).store_a(&InputsBwd[first]->NeuronsD1[hw]);
										mul_add(neuronsD1, VecFloat().load_a(&InputsFwd[first]->Neurons[hw]), VecFloat().load_a(&InputsBwd[second]->NeuronsD1[channelOffset])).store_a(&InputsBwd[second]->NeuronsD1[channelOffset]);
									}
								}
							});
						else
							for_i(batchSize, threads, [=](UInt n)
							{
								for (auto c = 0ull; c < C; c++)
								{
									...
									{
										InputsBwd[first]->NeuronsD1[hw] += NeuronsD1[hw] * InputsFwd[second]->Neurons[channelOffset];
										InputsBwd[second]->NeuronsD1[channelOffset] += NeuronsD1[hw] * InputsFwd[first]->Neurons[hw];
									}
								}
							});
#ifdef DNN_STOCHASTIC
					...
					if (!plain)
						for_i(batchSize, threads, [=](UInt n)
						{
							...
							for (auto c = 0ull; c < PaddedC; c += VectorSize)
							{
								...
								{
									neuronsD1.load_a(&NeuronsD1[hw + outputOffset]);
									mul_add(neuronsD1, InputsFwd[second]->Neurons[hw + channelOffset], VecFloat().load_a(&InputsBwd[first]->NeuronsD1[hw + outputOffset])).store_a(&InputsBwd[first]->NeuronsD1[hw + outputOffset]);
									//mul_add(neuronsD1, VecFloat().load_a(&InputsFwd[first]->Neurons[hw + outputOffset]), InputsBwd[second]->NeuronsD1[hw + channelOffset]).store_a(&InputsBwd[second]->NeuronsD1[hw + channelOffset]);
									InputsBwd[second]->NeuronsD1[hw + channelOffset] += horizontal_add(neuronsD1 * VecFloat().load_a(&InputsFwd[first]->Neurons[hw + outputOffset]));
								}
							}
						});
					else
						for_i(batchSize, threads, [=](UInt n)
						{
							...
							for (auto c = 0ull; c < C; c++)
							{
								...
								for (auto hw = 0ull; hw < HW(); hw++)
								{
									InputsBwd[first]->NeuronsD1[hw + outputOffset] += NeuronsD1[hw + outputOffset] * InputsFwd[second]->Neurons[hw + channelOffset];
									InputsBwd[second]->NeuronsD1[hw + channelOffset] += NeuronsD1[hw + outputOffset] * InputsFwd[first]->Neurons[hw + outputOffset];
								}
							}
						});
				}
