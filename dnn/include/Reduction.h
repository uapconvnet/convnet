			...
			else
			{
				...
				if (!plain)
					for_i(batchSize, threads, [=](UInt n)
					{
						...
						if (InputLayer->PaddedC == InputLayer->C)
							...
							for (auto c = 0ull; c < InputLayer->C; c++)
								for (auto h = 0ull; h < H; h++)
									for (auto w = 0ull; w < W; w++)
										InputLayer->NeuronsD1[InputLayer->OffsetPaddedMem(n, c, h, w)] += NeuronsD1[OffsetPaddedMem(n, 0, h, w)] / Float(InputLayer->C);
					});
				else
					for_i(batchSize, threads, [=](UInt n)
					{
						...
						if (InputLayer->PaddedC == InputLayer->C)
							for (auto c = 0ull; c < InputLayer->PaddedC; c += VectorSize)
							{
								...
								for (auto hw = 0ull; hw < strideHW; hw += VectorSize)
								{
									...
									auto neurons = VecFloat().load_a(&Neurons[hw + outputOffset]);
									auto neuronsD1 = VecFloat().load_a(&NeuronsD1[hw + outputOffset]);

									if_add(inputNeurons == Neurons[(hw / VectorSize) + outputOffset], inputNeuronsD1, neuronsD1).store_a(&InputLayer->NeuronsD1[hw + inputOffset]);
								}
							}
						else
							for (auto c = 0ull; c < InputLayer->C; c++)
