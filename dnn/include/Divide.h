			...
			if (batchSize == 1)
			{
				if (EqualDimensions(Inputs))
				{
					if (!plain)
					{
						...
						{
							mul_add(approx_recipr(VecFloat().load_a(&InputsFwd[1]->Neurons[cdhw])), VecFloat().load_a(&NeuronsD1[cdhw]), VecFloat().load_a(&InputsBwd[0]->NeuronsD1[cdhw])).store_a(&InputsBwd[0]->NeuronsD1[cdhw]);
							mul_add(VecFloat().load_a(&Inputs[0]->Neurons[cdhw]) * VecFloat().load_a(&NeuronsD1[cdhw]), approx_recipr(square(VecFloat().load_a(&InputsFwd[1]->Neurons[cdhw]))), VecFloat().load_a(&InputsBwd[1]->NeuronsD1[cdhw])).store_a(&InputsBwd[1]->NeuronsD1[cdhw]);
						}
					}
					else
					{
						...
						for (auto cdhw = 0ull; cdhw < CDHW(); cdhw++)
						{
							...
							InputsBwd[1]->NeuronsD1[cdhw] += NeuronsD1[cdhw] * InputsFwd[0]->Neurons[cdhw] / Square<Float>(InputsFwd[1]->Neurons[cdhw]);
						}
					}
				}
				else
					...
					if (!plain)
					{
						...
						for (auto c = 0ull; c < PaddedC; c += VectorSize)
						{
							...
							for (auto hw = 0ull; hw < strideHW; hw += VectorSize)
							{
								...
								mul_add(neuronsD1, approx_recipr(VecFloat().load_a(&InputsFwd[second]->Neurons[c])), VecFloat().load_a(&InputsBwd[first]->NeuronsD1[hw + outputOffset])).store_a(&InputsBwd[first]->NeuronsD1[hw + outputOffset]);
								mul_add(neuronsD1 * VecFloat().load_a(&InputsFwd[first]->Neurons[hw + outputOffset]), approx_recipr(square(VecFloat().load_a(&InputsFwd[second]->NeuronsD1[c]))), VecFloat().load_a(&InputsBwd[second]->NeuronsD1[c])).store_a(&InputsBwd[second]->NeuronsD1[c]);
							}
						}
					}
					else
					{
						for (auto c = 0ull; c < C; c++)
						{
							...
							for (auto hw = 0ull; hw < HW(); hw++)
							{
								...
								InputsBwd[second]->NeuronsD1[c] += NeuronsD1[hw + outputOffset] * InputsFwd[first]->Neurons[hw + outputOffset] / Square<Float>(InputsFwd[second]->Neurons[c]);
							}
						}
				...
				if (EqualDimensions(Inputs))
				{
					if (!plain)
					{
						for_i(batchSize, threads, [=](UInt n)
						{
							...
							{
								mul_add(approx_recipr(VecFloat().load_a(&InputsFwd[1]->Neurons[cdhw])), VecFloat().load_a(&NeuronsD1[cdhw]), VecFloat().load_a(&InputsBwd[0]->NeuronsD1[cdhw])).store_a(&InputsBwd[0]->NeuronsD1[cdhw]);
								mul_add(VecFloat().load_a(&InputsFwd[0]->Neurons[cdhw]) * VecFloat().load_a(&NeuronsD1[cdhw]), approx_recipr(square(VecFloat().load_a(&InputsFwd[1]->Neurons[cdhw]))), VecFloat().load_a(&InputsBwd[1]->NeuronsD1[cdhw])).store_a(&InputsBwd[1]->NeuronsD1[cdhw]);
							}
						});
					}
					else
					...
					if (!plain)
					{
						for_i(batchSize, threads, [=](UInt n)
						{
							...
							for (auto c = 0ull; c < PaddedC; c += VectorSize)
							{
								...
								{
									neuronsD1.load_a(&NeuronsD1[hw + outputOffset]);
									mul_add(neuronsD1, approx_recipr(VecFloat().load_a(&InputsFwd[second]->Neurons[channelOffset])), VecFloat().load_a(&InputsBwd[first]->NeuronsD1[hw + outputOffset])).store_a(&InputsBwd[first]->NeuronsD1[hw + outputOffset]);
									mul_add(neuronsD1 * VecFloat().load_a(&InputsFwd[first]->Neurons[hw + outputOffset]), approx_recipr(square(VecFloat().load_a(&InputsFwd[second]->NeuronsD1[channelOffset]))), VecFloat().load_a(&InputsBwd[second]->NeuronsD1[channelOffset])).store_a(&InputsBwd[second]->NeuronsD1[channelOffset]);
								}
							}
						});
					}
					else
					{
						for_i(batchSize, threads, [=](UInt n)
						{
							for (auto c = 0ull; c < C; c++)
							{
								...
								for (auto hw = 0ull; hw < HW(); hw++)
								{
									InputsBwd[first]->NeuronsD1[hw + outputOffset] += NeuronsD1[hw + outputOffset] / InputsFwd[second]->Neurons[channelOffset];
									InputsBwd[second]->NeuronsD1[channelOffset] += NeuronsD1[hw + outputOffset] * InputsFwd[first]->Neurons[hw + outputOffset] / Square<Float>(InputsFwd[second]->Neurons[channelOffset]);
								}
							}
						});
					}
