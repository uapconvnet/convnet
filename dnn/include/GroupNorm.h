...
namespace dnn
{
	...
	private:
		...
		std::unique_ptr<dnnl::group_normalization_forward::primitive_desc> fwdInferenceDesc;
		std::unique_ptr<dnnl::group_normalization_forward::primitive_desc> fwdTrainingDesc;
		std::unique_ptr<dnnl::group_normalization_backward::primitive_desc> bwdDesc;
		std::unique_ptr<dnnl::binary::primitive_desc> bwdAddDesc;
#ifdef DNN_CACHE_PRIMITIVES
		std::unique_ptr<dnnl::group_normalization_forward> fwdInference;
		std::unique_ptr<dnnl::group_normalization_forward> fwdTraining;
	...
	public:
		...
		void InitializeDescriptors(const UInt batchSize) final override
		{
			...
			const auto flagsInference = Scaling ?
					dnnl::normalization_flags::use_global_stats | dnnl::normalization_flags::use_scale | dnnl::normalization_flags::use_shift
					: dnnl::normalization_flags::use_global_stats;
			
            fwdInferenceDesc = std::make_unique<dnnl::group_normalization_forward::primitive_desc>(dnnl::group_normalization_forward::primitive_desc(Device.engine, dnnl::prop_kind::forward_inference, *DstMemDesc, *DstMemDesc, Groups, Eps, flagsInference));

			
            const auto flagsTraining = Scaling ?
					dnnl::normalization_flags::use_scale | dnnl::normalization_flags::use_shift
					: static_cast<dnnl::normalization_flags>(0U);

			fwdTrainingDesc = std::make_unique<dnnl::group_normalization_forward::primitive_desc>(dnnl::group_normalization_forward::primitive_desc(Device.engine, dnnl::prop_kind::forward_training, *DstMemDesc, *DstMemDesc, Groups, Eps, flagsTraining));

#ifdef DNN_CACHE_PRIMITIVES
			fwdInference = std::make_unique<dnnl::group_normalization_forward>(dnnl::group_normalization_forward(*fwdInferenceDesc));
            fwdTraining = std::make_unique<dnnl::group_normalization_forward>(dnnl::group_normalization_forward(*fwdTrainingDesc));
#endif

			Mean.resize(fwdInferenceDesc->mean_desc().get_size() / sizeof(Float), Float(0));
			Variance.resize(fwdInferenceDesc->variance_desc().get_size() / sizeof(Float), Float(1));

			reorderFwdSrc = fwdInferenceDesc->src_desc() != *InputLayer->DstMemDesc;
			
			bwdDesc = std::make_unique<dnnl::group_normalization_backward::primitive_desc>(dnnl::group_normalization_backward::primitive_desc(Device.engine, Scaling ? dnnl::prop_kind::backward : dnnl::prop_kind::backward_data, *DiffDstMemDesc, *DiffDstMemDesc, *DstMemDesc, Groups, Eps, flagsTraining, *fwdTrainingDesc));
		...
		void ForwardProp(const UInt batchSize, const bool training) final override
		{
			if (!training)
			{
				...
				auto srcMem = reorderFwdSrc ? dnnl::memory(fwdInferenceDesc->src_desc(), Device.engine) : memSrc;
				if (reorderFwdSrc)
				{
				...
				auto memMean = dnnl::memory(fwdInferenceDesc->mean_desc(), Device.engine, Mean.data());
				auto memVariance = dnnl::memory(fwdInferenceDesc->variance_desc(), Device.engine, Variance.data());
				auto dstMem = reorderFwdSrc ? dnnl::memory(fwdInferenceDesc->dst_desc(), Device.engine) : dnnl::memory(fwdInferenceDesc->dst_desc(), Device.engine, Neurons.data());
...
#ifdef DNN_CACHE_PRIMITIVES
					fwdInference->execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_SCALE, memScale }, { DNNL_ARG_SHIFT, memShift }, { DNNL_ARG_DST, dstMem } });
#endif
					dnnl::group_normalization_forward(*fwdInferenceDesc).execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_SCALE, memScale }, { DNNL_ARG_SHIFT, memShift }, { DNNL_ARG_DST, dstMem } });
				}
				else
#ifdef DNN_CACHE_PRIMITIVES
					fwdInference->execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_DST, dstMem } });
#else
					dnnl::group_normalization_forward(*fwdInferenceDesc).execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_DST, dstMem } });
#endif
				Device.stream.wait();
			...
			{
				auto memSrc = dnnl::memory(*InputLayer->DstMemDesc, Device.engine, InputLayer->Neurons.data());
				auto srcMem = reorderFwdSrc ? dnnl::memory(fwdTrainingDesc->src_desc(), Device.engine) : memSrc;
				if (reorderFwdSrc)
				{
					dnnl::reorder(memSrc, srcMem).execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_FROM, memSrc}, { DNNL_ARG_TO, srcMem } });
					Device.stream.wait();
				...
				auto memMean = dnnl::memory(fwdTrainingDesc->mean_desc(), Device.engine, Mean.data());
				auto memVariance = dnnl::memory(fwdTrainingDesc->variance_desc(), Device.engine, Variance.data());
				auto dstMem = reorderFwdSrc ? dnnl::memory(fwdTrainingDesc->dst_desc(), Device.engine) : dnnl::memory(fwdTrainingDesc->dst_desc(), Device.engine, Neurons.data());
...
#ifdef DNN_CACHE_PRIMITIVES
					fwdTraining->execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_SCALE, memScale }, { DNNL_ARG_SHIFT, memShift }, { DNNL_ARG_DST, dstMem } });
#else
					dnnl::group_normalization_forward(*fwdTrainingDesc).execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_SCALE, memScale }, { DNNL_ARG_SHIFT, memShift }, { DNNL_ARG_DST, dstMem } });
#endif
				}
				else
#ifdef DNN_CACHE_PRIMITIVES
					fwdTraining->execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_DST, dstMem } });
#else
					dnnl::group_normalization_forward(*fwdTrainingDesc).execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_DST, dstMem } });
#endif
				Device.stream.wait();
...
#else
				dnnl::group_normalization_backward(*bwdDesc).execute(Device.stream, std::unordered_map<int, dnnl::memory>{ { DNNL_ARG_SRC, srcMem }, { DNNL_ARG_DIFF_DST, diffDstMem }, { DNNL_ARG_MEAN, memMean }, { DNNL_ARG_VARIANCE, memVariance }, { DNNL_ARG_DIFF_SRC, diffSrcMem } });
#endif
			Device.stream.wait();
