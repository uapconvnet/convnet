...
// this memcpy, as well.
//
// ...If for some reason you absolutely, desperately need to use fast_memcpy
// instead of fast_memmove, and you need to use it on overlapping areas, enable
// the below definition. It will check for overlap and automatically redirect to
// fast_memmove if overlap is found.
...
//
// The "numbytes" argument in fast_memcpy and memcpy_large is just the total
...
void * fast_memcpy(void *dest, void *src, size_t numbytes)
{
  ...
  {
    returnval = fast_memmove(dest, src, numbytes);
    return returnval;
  }
#endif
