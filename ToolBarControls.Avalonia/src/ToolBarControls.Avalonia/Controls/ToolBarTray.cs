...
/// </summary>
public partial class ToolBarTray : Control, IAddChild
{
    /// <inheritdoc />
                        ...
                        if (DoubleUtil.LessThan(thumbChange, 0)) // Move left/up
                        {
                            ...
                            else if (toolBarIndex > 0) // Swap toolbars
                            {
                                ...
                                else
                                {
                                    ...
                                    if (fHorizontal)
                                    {
                                        if (DoubleUtil.LessThan(toolBarsTotalMinimum,
                        ...
                        else // Move right/down
                        {
                            ...
                            else
                            {
                                if (toolBarIndex < band.Count - 1) // Swap toolbars
                                {
                                    ...
                                    if (DoubleUtil.GreaterThanOrClose(
    ...
    private void ShrinkToolBars(List<ToolBar> band, int startIndex, int endIndex, double shrinkAmount)
    {
        if (Orientation == Orientation.Horizontal)
        {
            ...
            {
                var toolBar = band[i];
                if (DoubleUtil.GreaterThanOrClose(toolBar.Bounds.Width - shrinkAmount, toolBar.MinLength))
                {
        ...
        else
        {
            ...
            {
                var toolBar = band[i];
                if (DoubleUtil.GreaterThanOrClose(toolBar.Bounds.Height - shrinkAmount, toolBar.MinLength))
                {
    ...
    private void ExpandToolBars(List<ToolBar> band, int startIndex, int endIndex, double expandAmount)
    {
        if (Orientation == Orientation.Horizontal)
        {
            ...
            {
                var toolBar = band[i];
                if (DoubleUtil.LessThanOrClose(toolBar.Bounds.Width + expandAmount, toolBar.MaxLength))
                {
        ...
        else
        {
            ...
            {
                var toolBar = band[i];
                if (DoubleUtil.LessThanOrClose(toolBar.Bounds.Height + expandAmount, toolBar.MaxLength))
                {
