...
/// </summary>
public partial class ToolBarPanel : StackPanel, IDisposable
{
    private ToolBar? _toolBar;

    internal double MinLength { get; private set; }
    internal double MaxLength { get; private set; }
    internal Controls? GeneratedItemsCollection { get; private set; }
    ...
    public void Dispose()
    {
        ...
        GeneratedItemsCollection?.Clear();
        Children.Clear();
    ...
    protected override void OnInitialized()
    {
        ...
        if (TemplatedParent is ToolBar toolBar)
        {
            ...
            this[!OrientationProperty] = new Binding(nameof(ToolBar.Orientation))
            {
                RelativeSource = new RelativeSource(RelativeSourceMode.TemplatedParent)
            };

            if (GeneratedItemsCollection == null)
                GeneratedItemsCollection = new();
            else
                GeneratedItemsCollection.Clear();
    ...
    protected override Size MeasureOverride(Size constraint)
    {
        ...
        var hasAlwaysOverflowItems = MeasureGeneratedItems( /* asNeeded = */ false,
            layoutSlotSize,
            horizontal,
            maxExtent,
            ref stackDesiredSize,
            out overflowExtent);
        ...
        maxExtent = Math.Max(maxExtent, MinLength);

        stackDesiredSize = new Size();
        // This second call will measure all of the AsNeeded elements and place
        // them in the appropriate location.
        var hasAsNeededOverflowItems = MeasureGeneratedItems( /* asNeeded = */ true,
            layoutSlotSize,
            horizontal,
            maxExtent,
            ref stackDesiredSize,
    ...
    {
        if (_toolBar == null || GeneratedItemsCollection == null)
            return;
        ...
        void Remove(int index, int count)
        {
            ...
            {
                var control = GeneratedItemsCollection[index + i];
            ...
            GeneratedItemsCollection.RemoveRange(index, count);

            var childCount = GeneratedItemsCollection.Count;

            for (var i = index; i < childCount; ++i)
                generator.ItemContainerIndexChanged(GeneratedItemsCollection[i], i + count, i);
        }
    }

    private bool MeasureGeneratedItems(bool asNeededPass,
        Size constraint,
        bool horizontal,
        double maxExtent,
        ...
        out double overflowExtent)
    {
        ...
        if (GeneratedItemsCollection == null || _toolBar == null)
            return false;

        var minVisibleItemsCount = _toolBar.MinVisibleItemsCount;
        var overflowMode = _toolBar.OverflowMode;
        ...
        foreach (var child in GeneratedItemsCollection)
        {
            ...
            else
            {
                ...
                {
                    if (childrenIndex < minVisibleItemsCount)
                    {
                        var childDesiredSize = child.DesiredSize;
    ...
    private void Add(int index, IEnumerable items)
    {
        ...
        for (; i < childCount; ++i)
            generator.ItemContainerIndexChanged(GeneratedItemsCollection[i], i - delta, i);
    }
    ...
    private void InsertContainer(object item, int index)
    {
        ...
        GeneratedItemsCollection.Insert(index, container);
